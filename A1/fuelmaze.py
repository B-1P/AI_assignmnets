...
class Fuelmaze(Problem):
    ...
    def successors(self, state):
        ...
        if(state.fuel > 0):
            ...
            if((state.pos[0] < self.width - 1) and (not (state.pos[0] + 1, state.pos[1]) in self.obstacles)):
                States.append(("Right", 1, FuelmazeState((state.pos[0] + 1, state.pos[1]), state.fuel - 1)))
                
            if((state.pos[0] > 0) and (not (state.pos[0] - 1, state.pos[1]) in self.obstacles)):
